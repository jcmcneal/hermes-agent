...
def _get_backend() -> str:
    ...
    if configured in {"parallel", "firecrawl", "tavily", "exa", "ollama", "searxng", "brave-free", "ddgs", "xai"}:
        return configured
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "parallel", "firecrawl", "tavily", "ollama", "searxng", "brave-free", "ddgs", "xai"}:
        return _is_backend_available(configured)
    return any(
        _is_backend_available(backend)
        for backend in ("exa", "parallel", "firecrawl", "tavily", "ollama", "searxng", "brave-free", "ddgs", "xai")
    )
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            print("   Using Ollama Cloud API (https://ollama.com)")
        ...
        print(
            "Set one of: OLLAMA_API_KEY, EXA_API_KEY, PARALLEL_API_KEY, TAVILY_API_KEY, "
